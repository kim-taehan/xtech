...
// Inline tests parked under `cfg(any())`: same drift situation as
// `requests/chat.rs::tests`. Re-enable after the chat path is exercised and
// the new ResponseEvent::Completed { end_turn } / FunctionCall { namespace }
// shapes are propagated through the test fixtures.
#[cfg(any())]
mod tests {
    use super::*;
