...
const PROTECTED_METADATA_XTECH_PATH_NAME: &str = ".xtech";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_XTECH_PATH_NAME,
];
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        ...
        || name == OsStr::new(PROTECTED_METADATA_XTECH_PATH_NAME)
}
...
impl FileSystemSandboxPolicy {
    ...
    pub fn workspace_write(
        ...
        append_default_read_only_project_root_subpath_if_no_explicit_rule(&mut entries, ".xtech");
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .codex / .xtech read-only to the
    ...
    let top_level_xtech = writable_root.join(PROTECTED_METADATA_XTECH_PATH_NAME);
    if protect_missing_dot_codex || top_level_xtech.as_path().is_dir() {
        subpaths.push(top_level_xtech);
    }
...
mod tests {
    ...
    fn legacy_workspace_write_projection_preserves_symbolic_project_root() {
        ...
        assert_eq!(
            ...
            FileSystemSandboxPolicy::restricted(vec![
                ...
                FileSystemSandboxEntry {
                    path: FileSystemPath::Special {
                        value: FileSystemSpecialPath::project_roots(Some(".xtech".into())),
                    },
                    access: FileSystemAccessMode::Read,
                },
            ])
    ...
    fn filesystem_policy_blocks_protected_metadata_path_writes_by_default() {
        ...
        let dot_xtech_config = cwd.path().join(".xtech").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_xtech_config, cwd.path()));
        ...
        assert!(!writable_roots[0].is_path_writable(&dot_xtech_config));
    }
