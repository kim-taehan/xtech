...
| 항목 | 값 | 반영 위치 |
| --- | --- | --- |
| 호스트/baseURL | `http://<gateway-host>/v1` | 환경변수 `CODEX_OSS_BASE_URL` (코드/Git 에 박지 않음) |
| 모델 태그 | `qwen3.5-27b` | `codex-rs/ollama/src/lib.rs:16` `DEFAULT_OSS_MODEL` |
| 인증 토큰 | `sk-davis-XX-XXXXXXXX` | 환경변수 `OLLAMA_API_KEY` (코드/Git 에 박지 않음) |
...
`http://<gateway-host>/` 는 nginx/1.29.4 로 응답하고 OpenAI 호환 `/v1/*` 라우트를 노출한다. 무인증 / 잘못된 키로 `/v1/responses` 와 `/v1/chat/completions` 둘 다 `401 auth_error` 를 반환한다. **`/v1/responses` 가 404 가 아닌 401 이라는 것은 라우트가 살아있다는 강한 신호** — Codex 의 Responses API 와 그대로 호환 가능. 따라서 Codex 에 `wire_api = "chat"` 을 부활시킬 필요 없음.
...
curl -sS -o /tmp/r -w "HTTP %{http_code}\n" -X POST \
  http://<gateway-host>/v1/responses \
...
```bash
export CODEX_OSS_BASE_URL=http://<gateway-host>/v1
export OLLAMA_API_KEY=sk-davis-XX-XXXXXXXX
